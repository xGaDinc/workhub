    ...
    <div className="min-h-screen relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-4 md:py-6">
          <div className="flex justify-between items-center gap-2">
            <div className="flex items-center gap-2 md:gap-3">
              <div className="w-9 h-9 md:w-10 md:h-10 bg-gradient-to-br from-amber-400 via-amber-500 to-yellow-600 rounded-full flex items-center justify-center shadow-lg shadow-amber-500/30 flex-shrink-0">
                <svg className="w-4 h-4 md:w-5 md:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                </svg>
              </div>
              <h1 className="text-lg md:text-xl font-bold bg-gradient-to-r from-amber-200 via-amber-100 to-yellow-200 bg-clip-text text-transparent">
                Проекты
              </h1>
            </div>
            
            <div className="flex items-center gap-2 md:gap-3">
              <div className="hidden sm:flex items-center gap-2 px-3 md:px-4 py-2 bg-white/5 backdrop-blur-xl rounded-full border border-white/10">
                ...
                <span className="text-sm text-amber-100/90 font-medium hidden md:inline">{user.name}</span>
                {user.is_admin && (
              ...
              <button 
                onClick={onLogout} 
                className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-red-500/10 backdrop-blur-xl hover:bg-red-500/20 text-red-400 transition-all duration-300 flex items-center justify-center border border-red-500/20"
                title="Выйти"
              >
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-8 relative z-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
          <h2 className="text-xl md:text-2xl font-bold text-white">Ваши проекты</h2>
          <button
            onClick={() => setShowModal(true)}
            className="px-4 md:px-5 py-2 md:py-2.5 text-sm bg-gradient-to-r from-amber-500 to-yellow-600 text-slate-900 rounded-2xl font-bold hover:from-amber-600 hover:to-yellow-700 transform hover:scale-[1.02] transition-all duration-300 shadow-lg flex items-center gap-2"
          >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {projectList.map((project) => (
              <div
                key={project.id}
                className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 md:p-5 hover:bg-white/10 transition-all cursor-pointer group"
